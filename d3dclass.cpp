						  ...
						  float screenDepth, float screenNear)
{
    ...
    swapChainDesc.BufferDesc.Width = screenWidth;						// 缓冲区大小
    swapChainDesc.BufferDesc.Height = screenHeight;
    swapChainDesc.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;		// 缓冲区格式， 这里为32bit
	swapChainDesc.BufferDesc.RefreshRate.Numerator = (m_vsync_enabled) ? numerator : 0;		// 刷新率
    ...
    swapChainDesc.OutputWindow = hwnd;									// 要渲染的窗口句柄

    swapChainDesc.SampleDesc.Count = 1;									// 设置采样数，这里只设置一个, 不开启多重采样
    swapChainDesc.SampleDesc.Quality = 0;
	swapChainDesc.Windowed = ~fullscreen;								// 设置为窗口还是全屏模式
